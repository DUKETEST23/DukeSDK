 ...
 */
public final class LocalDataManager_SDK {
    final static String TAG = LocalDataManager_SDK.class.getSimpleName();
    ...
    public static String getLastAuthUser(final Context context, final String clientId) {
        ...
        try {
            SharedPreferences localCache
                    = context.getSharedPreferences(ClientConstants_SDK.APP_LOCAL_CACHE, Context.MODE_PRIVATE);
            String lastAuthUserKey = String.format(Locale.US, "%s.%s.%s",
                    ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, ClientConstants_SDK.APP_LAST_AUTH_USER);
    ...
    /**
     * Returns cached tokens for a user as a {@link AuthUserSession_SDK}.
     *
     ...
     */
    public static AuthUserSession_SDK getCachedSession(final Context context,
                                                       final String clientId,
                                                       final String username,
                                                       final Set<String> scopes) {
        AuthUserSession_SDK session = new AuthUserSession_SDK(null, null, null);
        if (username != null) {
            ...
            String cachedIdTokenKey = String.format(Locale.US, "%s.%s.%s.%s",
                    ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, ClientConstants_SDK.TOKEN_TYPE_ID);
            String cachedAccessTokenKey = String.format(Locale.US, "%s.%s.%s.%s",
                    ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, ClientConstants_SDK.TOKEN_TYPE_ACCESS);
            String cachedRefreshTokenKey = String.format(Locale.US, "%s.%s.%s.%s",
                    ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, ClientConstants_SDK.TOKEN_TYPE_REFRESH);
            String cachedTokenScopes = String.format(Locale.US, "%s.%s.%s.%s",
                    ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, ClientConstants_SDK.TOKEN_KEY_SCOPES);
            ...
            try {
                ...
                IdToken_SDK idTokenSDK =
                        new IdToken_SDK(localCache.getString(cachedIdTokenKey, null));

                AccessToken_SDK accessTokenSDK =
                        new AccessToken_SDK(localCache.getString(cachedAccessTokenKey, null));

                RefreshToken_SDK refreshTokenSDK =
                        new RefreshToken_SDK(localCache.getString(cachedRefreshTokenKey, null));

                session = new AuthUserSession_SDK(idTokenSDK, accessTokenSDK, refreshTokenSDK);
     ...
     * @return {@link AuthUserSession_SDK}.
     */
    public static AuthUserSession_SDK getCachedSession(final AWSKeyValueStore awsKeyValueStore,
        ...
        if (username != null) {
            ...
            try {
                ...
                // Scopes match, return the cached tokens
                IdToken_SDK idTokenSDK =
                        new IdToken_SDK(awsKeyValueStore.get(cachedIdTokenKey));

                AccessToken_SDK accessTokenSDK =
                        new AccessToken_SDK(awsKeyValueStore.get(cachedAccessTokenKey));

                RefreshToken_SDK refreshTokenSDK =
                        new RefreshToken_SDK(awsKeyValueStore.get(cachedRefreshTokenKey));
     ...
     * @param clientId Required: Cognito App/Client Id.
     * @param username Required: The username.
     * @param session  Required: User tokens as {@link AuthUserSession_SDK}.
     */
    public static void cacheSession(final Context context,
        ...
        String cachedTokenTypeKey = String.format(Locale.US, "%s.%s.%s.%s",
                ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, ClientConstants_SDK.TOKEN_KEY_TYPE);
        String cachedTokenScopes = String.format(Locale.US, "%s.%s.%s.%s",
                ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, ClientConstants_SDK.TOKEN_KEY_SCOPES);
        String lastAuthUserKey = String.format(Locale.US, "%s.%s.%s",
                ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, ClientConstants_SDK.APP_LAST_AUTH_USER);
        ...
        try {
            SharedPreferences.Editor localCacheEditor =
                    context.getSharedPreferences(ClientConstants_SDK.APP_LOCAL_CACHE, Context.MODE_PRIVATE).edit();
            localCacheEditor.putString(cachedTokenTypeKey, ClientConstants_SDK.SESSION_TYPE_JWT);
    ...
    public static void cacheSession(final AWSKeyValueStore awsKeyValueStore,
        ...
        try {
            awsKeyValueStore.put(cachedTokenTypeKey, ClientConstants_SDK.SESSION_TYPE_JWT);
    ...
    public static void cacheState(final Context context, final String key, final String proofKey, final Set<String> scopes) {
        try {
            ...
            localCacheEditor.putString(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_CHALLENGE, proofKey);
            localCacheEditor.putStringSet(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_SCOPES, scopes).apply();
        } catch (Exception e) {
            Log.e(TAG, "Failed while writing to SharedPreferences", e);
        }
    }
    ...
    public static void cacheState(final AWSKeyValueStore awsKeyValueStore,
                                  ...
                                  final Set<String> scopes) {
        try {
            awsKeyValueStore.put(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_CHALLENGE, proofKey);
            awsKeyValueStore.put(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_SCOPES, setToString(scopes));
        } catch (Exception e) {
            Log.e(TAG, "Failed while writing to SharedPreferences", e);
        }
    }
    ...
    public static String getCachedProofKey(final Context context, final String key) {
        try {
            SharedPreferences localCache =
                    context.getSharedPreferences(ClientConstants_SDK.APP_LOCAL_CACHE, 0);
            return localCache.getString(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_CHALLENGE, null);
        } catch (Exception e) {
            Log.e(TAG, "Failed to read from SharedPreferences", e);
        }
        return null;
    ...
    public static String getCachedProofKey(final AWSKeyValueStore awsKeyValueStore,
                                           ...
                                           final String key) {
        try {
            return awsKeyValueStore.get(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_CHALLENGE);
        } catch (Exception e) {
            Log.e(TAG, "Failed to read from SharedPreferences", e);
        }
        return null;
    ...
    public static Set<String> getCachedScopes(final Context context, final String key) {
        ...
        try {
            SharedPreferences localCache =
                    context.getSharedPreferences(ClientConstants_SDK.APP_LOCAL_CACHE, 0);
            return localCache.getStringSet(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_SCOPES, cachedSet);
        } catch (Exception e) {
            Log.e(TAG, "Failed to read from SharedPreferences", e);
        }
        return cachedSet;
    ...
    public static Set<String> getCachedScopes(final AWSKeyValueStore awsKeyValueStore,
        ...
        Set<String> cachedSet = new HashSet<String>();
        try {
            String cachedSetString = awsKeyValueStore.get(key + ClientConstants_SDK.DOMAIN_QUERY_PARAM_SCOPES);
    ...
    public static void cacheHasReceivedRedirect(AWSKeyValueStore awsKeyValueStore, Context context, String clientId, boolean hasReceivedRedirect) {
        ...
        try {
            String hasReceivedRedirectKey = String.format(Locale.US, "%s.%s.%s",
                    ClientConstants_SDK.APP_LOCAL_CACHE_KEY_PREFIX, clientId, ClientConstants_SDK.APP_HAS_RECIEVED_REDIRECT);
