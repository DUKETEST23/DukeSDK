...
import com.dukeai.awsauth_sdk.exceptions_sdk.AuthNavigationException_SDK;
import com.dukeai.awsauth_sdk.exceptions_sdk.AuthServiceException_SDK;
import com.dukeai.awsauth_sdk.handlers_sdk.AuthHandler_SDK;
import com.dukeai.awsauth_sdk.util_sdk.AuthHttpClient_SDK;
import com.dukeai.awsauth_sdk.util_sdk.AuthHttpResponseParser_SDK;
import com.dukeai.awsauth_sdk.util_sdk.ClientConstants_SDK;
import com.dukeai.awsauth_sdk.util_sdk.LocalDataManager_SDK;
import com.dukeai.awsauth_sdk.util_sdk.Pkce_SDK;
...
/**
 * Local client for {@link Auth_SDK}.
 * <p>
 * Encapsulates user level operations, tokens {@link AuthUserSession_SDK}, handles
...
public class AuthClient_SDK {
     ...
     * Callback handler.
     */
    private AuthHandler_SDK userHandler;
    /**
    ...
    private final CustomTabsCallback customTabsCallback = new CustomTabsCallback() {
        ...
        public void onNavigationEvent(final int navigationEvent, final Bundle extras) {
            ...
            if (navigationEvent == ClientConstants_SDK.CHROME_NAVIGATION_CANCELLED) {
                final boolean hasReceivedRedirect = LocalDataManager_SDK.hasReceivedRedirect(pool.awsKeyValueStore,
                ...
                if (!hasReceivedRedirect) {
                    userHandler.onFailure(new AuthNavigationException_SDK("user cancelled"));
                    LocalDataManager_SDK.cacheHasReceivedRedirect(pool.awsKeyValueStore, context, pool.getAppId(), false);
                }
            }
        }
    };
    ...
    /**
     * Constructs {@link AuthClient_SDK} with no user name.
     *
     * @param context Required: The android application {@link Context}.
     * @param pool    Required: A reference to the parent, {@link Auth_SDK}.
     */
    protected AuthClient_SDK(final Context context, final Auth_SDK pool) {
     ...
     */
    protected AuthClient_SDK(final Context context, final Auth_SDK pool, final String username) {
    ...
    /**
     * Set callback handler for {@link AuthClient_SDK}.
     *
     * @param handler Required: {@link AuthHandler_SDK}.
     */
    protected void setUserHandler(final AuthHandler_SDK handler) {
        if (handler == null) {
     ...
     * To exchange the code for tokens, the {@link Auth_SDK#getTokens(Uri)} method will use the
    ...
    protected void getSession(final boolean showSignInIfExpired) {
        try {
            proofKey = Pkce_SDK.generateRandom();
            proofKeyHash = Pkce_SDK.generateHash(proofKey);
            state = Pkce_SDK.generateRandom();
        } catch (Exception e) {
            userHandler.onFailure(e);
        }

        // Look for cached tokens
        AuthUserSession_SDK session =
                LocalDataManager_SDK.getCachedSession(pool.awsKeyValueStore, context, pool.getAppId(), userId, pool.getScopes());
     ...
     */
    public void signOut() {
        LocalDataManager_SDK.clearCache(pool.awsKeyValueStore, context, pool.getAppId(), userId);
    ...
    public void getTokens(final Uri uri) {
        ...
        // The flag
        LocalDataManager_SDK.cacheHasReceivedRedirect(pool.awsKeyValueStore, context, pool.getAppId(), true);
        getTokens(uri, userHandler);
    }

    /**
     * Unbind {@link AuthClient_SDK#mCustomTabsServiceConnection}
     */
     ...
     * {@link AuthHandler_SDK#onFailure(Exception)} callback to report failure.
     ...
     * @param callback Required: {@link AuthHandler_SDK}.
     */
    private void getTokens(final Uri uri, final AuthHandler_SDK callback) {
        new Thread(new Runnable() {
            ...
            public void run() {
                final Uri fqdn = new Uri.Builder()
                        .scheme(ClientConstants_SDK.DOMAIN_SCHEME)
                        .authority(pool.getAppWebDomain())
                        .appendPath(ClientConstants_SDK.DOMAIN_PATH_OAUTH2)
                        .appendPath(ClientConstants_SDK.DOMAIN_PATH_TOKEN_ENDPOINT)
                        .build();

                String callbackState =
                        uri.getQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_STATE);

                if (callbackState != null) {
                    Set<String> tokenScopes = LocalDataManager_SDK.getCachedScopes(pool.awsKeyValueStore, context, callbackState);
                    String proofKeyPlain = LocalDataManager_SDK.getCachedProofKey(pool.awsKeyValueStore, context, callbackState);
                    ...
                    final String errorText =
                            uri.getQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_ERROR);

                    if (errorText != null) {
                        returnCallback = new Runnable() {
                            @Override
                            public void run() {
                                callback.onFailure(new AuthServiceException_SDK(errorText));
                            }
                        };
                    } else {
                        // Make http POST call
                        final AuthHttpClient_SDK httpClient = new AuthHttpClient_SDK();
                        ...
                        try {
                            ...
                            final AuthUserSession_SDK session = AuthHttpResponseParser_SDK.parseHttpResponse(response);
                            userId = session.getUsername();

                            // Cache tokens if successful
                            LocalDataManager_SDK.cacheSession(pool.awsKeyValueStore, context, pool.getAppId(), userId, session, tokenScopes);
     ...
     * @param callback    Required: {@link AuthHandler_SDK}.
     */
    private void refreshSession(final AuthUserSession_SDK session,
                                final String redirectUri,
                                final Set<String> tokenScopes,
                                final AuthHandler_SDK callback) {
        new Thread(new Runnable() {
            final Handler handler = new Handler(context.getMainLooper());
            Runnable returnCallback = new Runnable() {
                @Override
            ...
            public void run() {
                ...
                try {
                    String response =
                            httpClient.httpPost(new URL(fqdn.toString()), httpHeaderParams, httpBodyParams);
                    AuthUserSession_SDK parsedSession = AuthHttpResponseParser_SDK.parseHttpResponse(response);
                    final AuthUserSession_SDK refreshedSession = new AuthUserSession_SDK(
                    ...
                    // Cache session
                    LocalDataManager_SDK.cacheSession(pool.awsKeyValueStore, context, pool.getAppId(), username, refreshedSession, pool.getScopes());
                ...
                } catch (final AuthInvalidGrantException_SDK invg) {
                    returnCallback = new Runnable() {
                        @Override
                        public void run() {
                            launchCognitoAuth(redirectUri, tokenScopes);
    ...
    private Map<String, String> getHttpHeader() {
        ...
        httpHeaderParams.put(ClientConstants_SDK.HTTP_HEADER_PROP_CONTENT_TYPE,
                ClientConstants_SDK.HTTP_HEADER_PROP_CONTENT_TYPE_DEFAULT);
        ...
        if (pool.getAppSecret() != null) {
            ...
            httpHeaderParams.put(ClientConstants_SDK.HTTP_HEADER_TYPE_AUTHORIZE, "Basic "
                    + Pkce_SDK.encodeBase64(builder.toString()));
        }
    ...
    private Map<String, String> generateTokenExchangeRequest(final Uri redirectUri,
        ...
        httpBodyParams.put(ClientConstants_SDK.TOKEN_GRANT_TYPE,
                ClientConstants_SDK.TOKEN_GRANT_TYPE_AUTH_CODE);
        httpBodyParams.put(ClientConstants_SDK.DOMAIN_QUERY_PARAM_CLIENT_ID, pool.getAppId());
        httpBodyParams.put(ClientConstants_SDK.DOMAIN_QUERY_PARAM_REDIRECT_URI, pool.getSignInRedirectUri());
        httpBodyParams.put(ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_VERIFIER, proofKey);
        httpBodyParams.put(ClientConstants_SDK.TOKEN_AUTH_TYPE_CODE,
                redirectUri.getQueryParameter(ClientConstants_SDK.TOKEN_AUTH_TYPE_CODE));
    ...
    private Map<String, String> generateTokenRefreshRequest(final String redirectUri,
                                                            final AuthUserSession_SDK session) {
        Map<String, String> httpBodyParams = new HashMap<String, String>();
        httpBodyParams.put(ClientConstants_SDK.TOKEN_GRANT_TYPE, ClientConstants_SDK.HTTP_REQUEST_REFRESH_TOKEN);
        httpBodyParams.put(ClientConstants_SDK.DOMAIN_QUERY_PARAM_REDIRECT_URI, redirectUri);
        httpBodyParams.put(ClientConstants_SDK.DOMAIN_QUERY_PARAM_CLIENT_ID, pool.getAppId());
        httpBodyParams.put(ClientConstants_SDK.HTTP_REQUEST_REFRESH_TOKEN, session.getRefreshToken().getToken());
        final String userContextData = getUserContextData();
        if (userContextData != null) {
            httpBodyParams.put(ClientConstants_SDK.DOMAIN_QUERY_PARAM_USERCONTEXTDATA, userContextData);
        }
        return httpBodyParams;
    }
    ...
    private void launchCognitoAuth(final String redirectUri, final Set<String> tokenScopes) {
        ...
        Uri.Builder builder = new Uri.Builder()
                ...
                .authority(pool.getAppWebDomain())
                .appendPath(ClientConstants_SDK.DOMAIN_PATH_OAUTH2)
                .appendPath(ClientConstants_SDK.DOMAIN_PATH_SIGN_IN)
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_CLIENT_ID, pool.getAppId())
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_REDIRECT_URI, redirectUri)
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_RESPONSE_TYPE,
                        ClientConstants_SDK.AUTH_RESPONSE_TYPE_CODE)
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_CHALLENGE, proofKeyHash)
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_CHALLENGE_METHOD,
                        ClientConstants_SDK.DOMAIN_QUERY_PARAM_CODE_CHALLENGE_METHOD_SHA256)
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_STATE, state)
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_USERCONTEXTDATA, getUserContextData());

        //check if identity provider set as param.
        if (!TextUtils.isEmpty(pool.getIdentityProvider())) {
            builder.appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_IDENTITY_PROVIDER, pool.getIdentityProvider());
        }
        //check if idp identifier set as param.
        if (!TextUtils.isEmpty(pool.getIdpIdentifier())) {
            builder.appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_IDP_IDENTIFIER, pool.getIdpIdentifier());
        }
        ...
        if (noOfScopes > 0) {
            ...
            builder.appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_SCOPES, scopesStr);
        }

        final Uri fqdn = builder.build();
        LocalDataManager_SDK.cacheState(pool.awsKeyValueStore, context, state, proofKey, tokenScopes);
    ...
    private void launchSignOut(final String redirectUri) {
        Uri.Builder builder = new Uri.Builder()
                .scheme(ClientConstants_SDK.DOMAIN_SCHEME)
                .authority(pool.getAppWebDomain()).appendPath(ClientConstants_SDK.DOMAIN_PATH_SIGN_OUT)
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_CLIENT_ID, pool.getAppId())
                .appendQueryParameter(ClientConstants_SDK.DOMAIN_QUERY_PARAM_LOGOUT_URI, redirectUri);
        final Uri fqdn = builder.build();
        launchCustomTabs(fqdn);
    }
    ...
    private void launchCustomTabs(final Uri uri) {
        try {
            ...
            mCustomTabsIntent.intent.setPackage(ClientConstants_SDK.CHROME_PACKAGE);
    ...
    private void preWarmChrome() {
        ...
        boolean chromeState = CustomTabsClient.bindCustomTabsService(context,
                ClientConstants_SDK.CHROME_PACKAGE, mCustomTabsServiceConnection);
    }
}
