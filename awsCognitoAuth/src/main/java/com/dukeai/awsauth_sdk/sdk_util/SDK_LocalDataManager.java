 ...
 */
public final class SDK_LocalDataManager {
    final static String TAG = SDK_LocalDataManager.class.getSimpleName();
    ...
    public static String getLastAuthUser(final Context context, final String clientId) {
        ...
        try {
            SharedPreferences localCache
                    = context.getSharedPreferences(SDK_ClientConstants.APP_LOCAL_CACHE, Context.MODE_PRIVATE);
            String lastAuthUserKey = String.format(Locale.US, "%s.%s.%s",
                    SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, SDK_ClientConstants.APP_LAST_AUTH_USER);
    ...
    /**
     * Returns cached tokens for a user as a {@link SDK_AuthUserSession}.
     *
     ...
     */
    public static SDK_AuthUserSession getCachedSession(final Context context,
                                                       final String clientId,
                                                       final String username,
                                                       final Set<String> scopes) {
        SDK_AuthUserSession session = new SDK_AuthUserSession(null, null, null);
        if (username != null) {
            ...
            String cachedIdTokenKey = String.format(Locale.US, "%s.%s.%s.%s",
                    SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, SDK_ClientConstants.TOKEN_TYPE_ID);
            String cachedAccessTokenKey = String.format(Locale.US, "%s.%s.%s.%s",
                    SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, SDK_ClientConstants.TOKEN_TYPE_ACCESS);
            String cachedRefreshTokenKey = String.format(Locale.US, "%s.%s.%s.%s",
                    SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, SDK_ClientConstants.TOKEN_TYPE_REFRESH);
            String cachedTokenScopes = String.format(Locale.US, "%s.%s.%s.%s",
                    SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, SDK_ClientConstants.TOKEN_KEY_SCOPES);
            ...
            try {
                ...
                SDK_IdTokenSDK SDKIdToken =
                        new SDK_IdTokenSDK(localCache.getString(cachedIdTokenKey, null));

                SDK_AccessTokenSDK SDKAccessToken =
                        new SDK_AccessTokenSDK(localCache.getString(cachedAccessTokenKey, null));

                SDK_RefreshTokenSDK SDKRefreshToken =
                        new SDK_RefreshTokenSDK(localCache.getString(cachedRefreshTokenKey, null));

                session = new SDK_AuthUserSession(SDKIdToken, SDKAccessToken, SDKRefreshToken);
     ...
     * @return {@link SDK_AuthUserSession}.
     */
    public static SDK_AuthUserSession getCachedSession(final AWSKeyValueStore awsKeyValueStore,
        ...
        if (username != null) {
            ...
            try {
                ...
                // Scopes match, return the cached tokens
                SDK_IdTokenSDK SDKIdToken =
                        new SDK_IdTokenSDK(awsKeyValueStore.get(cachedIdTokenKey));

                SDK_AccessTokenSDK SDKAccessToken =
                        new SDK_AccessTokenSDK(awsKeyValueStore.get(cachedAccessTokenKey));

                SDK_RefreshTokenSDK SDKRefreshToken =
                        new SDK_RefreshTokenSDK(awsKeyValueStore.get(cachedRefreshTokenKey));
     ...
     * @param clientId Required: Cognito App/Client Id.
     * @param username Required: The username.
     * @param session  Required: User tokens as {@link SDK_AuthUserSession}.
     */
    public static void cacheSession(final Context context,
        ...
        String cachedTokenTypeKey = String.format(Locale.US, "%s.%s.%s.%s",
                SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, SDK_ClientConstants.TOKEN_KEY_TYPE);
        String cachedTokenScopes = String.format(Locale.US, "%s.%s.%s.%s",
                SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, username, SDK_ClientConstants.TOKEN_KEY_SCOPES);
        String lastAuthUserKey = String.format(Locale.US, "%s.%s.%s",
                SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, SDK_ClientConstants.APP_LAST_AUTH_USER);
        ...
        try {
            SharedPreferences.Editor localCacheEditor =
                    context.getSharedPreferences(SDK_ClientConstants.APP_LOCAL_CACHE, Context.MODE_PRIVATE).edit();
            localCacheEditor.putString(cachedTokenTypeKey, SDK_ClientConstants.SESSION_TYPE_JWT);
    ...
    public static void cacheSession(final AWSKeyValueStore awsKeyValueStore,
        ...
        try {
            awsKeyValueStore.put(cachedTokenTypeKey, SDK_ClientConstants.SESSION_TYPE_JWT);
    ...
    public static void cacheState(final Context context, final String key, final String proofKey, final Set<String> scopes) {
        try {
            ...
            localCacheEditor.putString(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_CODE_CHALLENGE, proofKey);
            localCacheEditor.putStringSet(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_SCOPES, scopes).apply();
        } catch (Exception e) {
            Log.e(TAG, "Failed while writing to SharedPreferences", e);
        }
    }
    ...
    public static void cacheState(final AWSKeyValueStore awsKeyValueStore,
                                  ...
                                  final Set<String> scopes) {
        try {
            awsKeyValueStore.put(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_CODE_CHALLENGE, proofKey);
            awsKeyValueStore.put(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_SCOPES, setToString(scopes));
        } catch (Exception e) {
            Log.e(TAG, "Failed while writing to SharedPreferences", e);
        }
    }
    ...
    public static String getCachedProofKey(final Context context, final String key) {
        try {
            SharedPreferences localCache =
                    context.getSharedPreferences(SDK_ClientConstants.APP_LOCAL_CACHE, 0);
            return localCache.getString(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_CODE_CHALLENGE, null);
        } catch (Exception e) {
            Log.e(TAG, "Failed to read from SharedPreferences", e);
        }
        return null;
    ...
    public static String getCachedProofKey(final AWSKeyValueStore awsKeyValueStore,
                                           ...
                                           final String key) {
        try {
            return awsKeyValueStore.get(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_CODE_CHALLENGE);
        } catch (Exception e) {
            Log.e(TAG, "Failed to read from SharedPreferences", e);
        }
        return null;
    ...
    public static Set<String> getCachedScopes(final Context context, final String key) {
        ...
        try {
            SharedPreferences localCache =
                    context.getSharedPreferences(SDK_ClientConstants.APP_LOCAL_CACHE, 0);
            return localCache.getStringSet(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_SCOPES, cachedSet);
        } catch (Exception e) {
            Log.e(TAG, "Failed to read from SharedPreferences", e);
        }
        return cachedSet;
    ...
    public static Set<String> getCachedScopes(final AWSKeyValueStore awsKeyValueStore,
        ...
        Set<String> cachedSet = new HashSet<String>();
        try {
            String cachedSetString = awsKeyValueStore.get(key + SDK_ClientConstants.DOMAIN_QUERY_PARAM_SCOPES);
    ...
    public static void cacheHasReceivedRedirect(AWSKeyValueStore awsKeyValueStore, Context context, String clientId, boolean hasReceivedRedirect) {
        ...
        try {
            String hasReceivedRedirectKey = String.format(Locale.US, "%s.%s.%s",
                    SDK_ClientConstants.APP_LOCAL_CACHE_KEY_PREFIX, clientId, SDK_ClientConstants.APP_HAS_RECIEVED_REDIRECT);
